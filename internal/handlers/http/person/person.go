...
type PersonHandler struct {
	Service interfaceperson.ServicePersonInterface
}

func NewPersonHandler(s interfaceperson.ServicePersonInterface) *PersonHandler {
	return &PersonHandler{Service: s}
}

func (h *PersonHandler) Create(ctx *gin.Context) {
	var req dto.PersonCreate
	logId := utils.GenerateLogId(ctx)
	logPrefix := fmt.Sprintf("[%s][PersonHandler][Create]", logId)
	...
	res := response.Response(http.StatusCreated, "Person created successfully", logId, data)
...
func (h *PersonHandler) GetByID(ctx *gin.Context) {
	id := ctx.Param("id")
	logId := utils.GenerateLogId(ctx)
	logPrefix := fmt.Sprintf("[%s][PersonHandler][GetByID]", logId)

	data, err := h.Service.GetByID(id)
	if err != nil {
		logger.WriteLog(logger.LogLevelError, fmt.Sprintf("%s; Service.GetByID; Error: %+v", logPrefix, err))
		res := response.Response(http.StatusNotFound, "Person not found", logId, nil)
		res.Error = err.Error()
		ctx.JSON(http.StatusNotFound, res)
		return
	}

	res := response.Response(http.StatusOK, "Get person successfully", logId, data)
	logger.WriteLog(logger.LogLevelDebug, fmt.Sprintf("%s; Response: %+v;", logPrefix, utils.JsonEncode(data)))
...
func (h *PersonHandler) GetAll(ctx *gin.Context) {
	logId := utils.GenerateLogId(ctx)
	logPrefix := fmt.Sprintf("[%s][PersonHandler][GetAll]", logId)
...
func (h *PersonHandler) Update(ctx *gin.Context) {
	id := ctx.Param("id")
	var req dto.PersonUpdate
	logId := utils.GenerateLogId(ctx)
	logPrefix := fmt.Sprintf("[%s][PersonHandler][Update]", logId)
	...
	res := response.Response(http.StatusOK, "Person updated successfully", logId, data)
	logger.WriteLog(logger.LogLevelDebug, fmt.Sprintf("%s; Response: %+v;", logPrefix, utils.JsonEncode(data)))
	ctx.JSON(http.StatusOK, res)
}
...
func (h *PersonHandler) Delete(ctx *gin.Context) {
	id := ctx.Param("id")
	logId := utils.GenerateLogId(ctx)
	logPrefix := fmt.Sprintf("[%s][PersonHandler][Delete]", logId)
	...
	res := response.Response(http.StatusOK, "Person deactivated successfully", logId, nil)
	logger.WriteLog(logger.LogLevelDebug, fmt.Sprintf("%s; Response: Person deactivated", logPrefix))
	ctx.JSON(http.StatusOK, res)
}
